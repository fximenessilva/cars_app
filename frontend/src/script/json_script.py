...
def delete_user(users, user_id):
    for user in users:
        if user["id"] == user_id:
            users.remove(user)
            return
    raise ValueError("No user with the specified ID was found")

def add_car_to_user(users, user_id, car_id):
    user = get_user(users, user_id)
    if car_id in user["favorite_cars"]:
        raise ValueError("The car is already assigned as a favorite to the user")
    user["favorite_cars"].append(car_id)

def delete_car_from_user(users, user_id, car_id):
    user = get_user(users, user_id)
    if car_id not in user["favorite_cars"]:
        raise ValueError("The car is not assigned as a favorite to the user")
